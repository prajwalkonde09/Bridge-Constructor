Shader "Water/Ocean_06_NoiseColor" {
    Properties {
        _WaterColor ("Water Color", Color) = (0.4926471,0.6011156,1,1)
        _NoiseColor ("Noise Color", Color) = (1,1,1,1) // ✅ NEW
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float4 _NoiseColor; // ✅ NEW
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_1855_cos = cos(node_1855_ang);
                float node_1855_sin = sin(node_1855_ang);
                ...
                float2 node_7411 = (node_1855+node_4825.g*float2(0,0.05));
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(node_7411, _MainTexture));

                float2 node_5898 = (node_1855+node_4825.g*float2(0,0.01));
                ...
                // ✅ APPLY NOISE COLOR HERE
                float noiseValue = _Noise_var.r;
                float3 coloredNoise = noiseValue * _NoiseColor.rgb;

                float2 node_2716 = (lerp(node_1855,float2(noiseValue,noiseValue),_Noiselevel)+node_4825.g*float2(0,0.02));
                float4 _FoamTexture_var = tex2D(_FoamTexture,TRANSFORM_TEX(node_2716, _FoamTexture));

                float2 node_3076 = lerp(sceneUVs,float2(noiseValue,noiseValue),_Noiselevel);
                ...
                // ✅ inject colored noise influence into final
                float3 emissive = saturate(_WaterColor.rgb * _MainTexture_var.rgb + coloredNoise);
